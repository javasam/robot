...
unsigned char d5[16] = {0x4e, 0x8a, 0x8a, 0x8a, 0x72};
unsigned char d6[16] = {0x7c, 0x92, 0x92, 0x92, 0x64};
unsigned char d7[16] = {0x06, 0x02, 0xe2, 0x12, 0x0e};
unsigned char d8[16] = {0x6c, 0x92, 0x92, 0x92, 0x6c};
unsigned char d9[16] = {0x4c, 0x92, 0x92, 0x92, 0x7c};
unsigned char A[16] = {0xfc, 0x22, 0x22, 0xfc};
unsigned char B[16] = {0xfe, 0x92, 0x92, 0x6c};
unsigned char C[16] = {0x7c, 0x82, 0x82, 0x44};
unsigned char D[16] = {0xfe, 0x82, 0x82, 0x7c};
unsigned char E[16] = {0xfe, 0x92, 0x92, 0x82};
unsigned char F[16] = {0xfe, 0x12, 0x12, 0x02};
unsigned char G[16] = {0x7c, 0x82, 0x92, 0x72};
unsigned char H[16] = {0xfe, 0x10, 0x10, 0xfe};
unsigned char I[16] = {0x82, 0xfe, 0x82};
unsigned char J[16] = {0x40, 0x80, 0x80, 0x7e};
unsigned char K[16] = {0xfe, 0x10, 0x28, 0xc6};
unsigned char L[16] = {0xfe, 0x80, 0x80, 0x80};
unsigned char M[16] = {0xfe, 0x04, 0x08, 0x04, 0xfe};
unsigned char N[16] = {0xfe, 0x08, 0x10, 0x20, 0xfe};
unsigned char O[16] = {0x7c, 0x82, 0x82, 0x7c};
...
void setup() {
  ...
  matrix_start();
  matrix_send(0x8A);  // Контроль дисплея, установка ширины импульса на 4/16 s
  matrix_end();
}
...
void matrix_display_column(unsigned char data, int column) {
  if (column >= 0 && column < 16) {  // Убедитесь, что колонка находится в пределах матрицы
    matrix_start();  
    matrix_send(0xC0 | column);  // Адресуем колонку
    matrix_send(data);           // Отправляем данные для этой колонки
    matrix_end();  
  }
}
...
//Convey data
void matrix_send(unsigned char send_data) {
